...
import type { CharacterNode, GraphData, RelationshipLink } from '../../types/graph';
import rawData from '../../data/relationships.json';
import { AVATAR_MAP } from '../../utils/avatars';
import { getGroupIconDataUrl, GROUP_ICON_SVGS } from '../../utils/groupIcons';
import { getGroupColor } from '../shared/GroupIcon';
...
export function GraphCanvas() {
    ...
    const fgRef = useRef<ForceGraphMethods<CharacterNode, RelationshipLink>>(undefined);
    const imagesRef = useRef<Record<string, HTMLImageElement>>({});
    const groupIconsRef = useRef<Record<string, HTMLImageElement>>({});
    ...
    useEffect(() => {
        ...
        // Pre-load group icons
        Object.keys(GROUP_ICON_SVGS).forEach((group) => {
            const color = getGroupColor(group);
            const img = new Image();
            img.src = getGroupIconDataUrl(group, color);
            groupIconsRef.current[group] = img;
        });
        // Default icon
        const defaultImg = new Image();
        defaultImg.src = getGroupIconDataUrl('Прочие', getGroupColor('Прочие'));
        groupIconsRef.current['Прочие'] = defaultImg;
    ...
    const nodeCanvasObject = useCallback((node: CharacterNode, ctx: CanvasRenderingContext2D, globalScale: number) => {
        ...
        // Draw Group Icon Sticker
        const groupIconImg = groupIconsRef.current[node.group] || groupIconsRef.current['Прочие'];
        if (groupIconImg && groupIconImg.complete) {
            const stickerSize = 4; // Reduced from 6
            const stickerX = x + w - stickerSize - 1;
            const stickerY = y + h - stickerSize - 1;

            ctx.save();
            // Background for sticker (circle) to make it pop
            ctx.fillStyle = '#ffffff';
            ctx.beginPath();
            ctx.arc(stickerX + stickerSize / 2, stickerY + stickerSize / 2, stickerSize / 2 + 0.5, 0, Math.PI * 2);
            ctx.fill();

            // Draw Icon
            ctx.drawImage(groupIconImg, stickerX, stickerY, stickerSize, stickerSize);
            ctx.restore();
        }
    ...
    const nodePointerAreaPaint = useCallback((node: CharacterNode, color: string, ctx: CanvasRenderingContext2D) => {
        const w = 24;
        const h = 28;
        ctx.fillStyle = color;
